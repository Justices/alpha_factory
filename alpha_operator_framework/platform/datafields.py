...
def retry_after_seconds(response: Any, *, fallback: float) -> float:
    ...
    try:
        ...
        try:
            return max(
                (
                    parsedate_to_datetime(str(raw)) - datetime.now(timezone.utc)
                ).total_seconds(),
                0.0,
            )
...
async def fetch_datafields(
    ...
    total: int | None = None
    while (total is None or len(rows) < total) and (
        ...
        for _ in range(max_retries + 1):
            response = brain_client.session.get(
                f"{brain_client.base_url}/data-fields", params=params
            )
            if response.status_code != 429:
                break
            await asyncio.sleep(
                retry_after_seconds(response, fallback=page_delay + 1.0)
            )
