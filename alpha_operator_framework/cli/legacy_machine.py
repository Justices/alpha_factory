...
def main() -> None:
    ...
    backup_p = sub.add_parser("storage-backup", help="为 YAML 配置的 SQLite 存储创建一致性备份")
    backup_p.add_argument("--config", default=str(DEFAULT_RUNTIME_CONFIG_PATH), help="运行时 YAML 配置文件")
    backup_p.add_argument("--destination", required=True, help="尚不存在的备份文件路径")
    backup_p.set_defaults(func=command_storage_backup)
    restore_p = sub.add_parser("storage-restore", help="从备份恢复 YAML 配置的 SQLite 存储")
    restore_p.add_argument("--config", default=str(DEFAULT_RUNTIME_CONFIG_PATH), help="运行时 YAML 配置文件")
    restore_p.add_argument("--backup", required=True, help="备份文件路径")
    restore_p.set_defaults(func=command_storage_restore)
    ...
    worker_p.add_argument("--round-id", help="待恢复的研究轮次；非 --watch 时必填")
...
def command_research_worker(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "watch", False):
        ...
        if getattr(args, "telemetry_file", None):
            JsonLinesTelemetrySink(Path(args.telemetry_file)).publish(runtime.telemetry)
        return
    if not getattr(args, "round_id", None):
...
def command_storage_backup(args: argparse.Namespace) -> None:
    """Create a consistent SQLite backup using only the configured storage."""
    from alpha_operator_framework.infrastructure.runtime_factory import storage_config
    from alpha_operator_framework.infrastructure.storage_backup import backup_sqlite_storage

    destination = backup_sqlite_storage(
        storage_config(Path(getattr(args, "config", DEFAULT_RUNTIME_CONFIG_PATH))),
        Path(args.destination),
    )
    print(f"storage backup created: {destination}")


def command_storage_restore(args: argparse.Namespace) -> None:
    """Restore a SQLite backup using only the configured storage."""
    from alpha_operator_framework.infrastructure.runtime_factory import storage_config
    from alpha_operator_framework.infrastructure.storage_backup import restore_sqlite_storage

    restore_sqlite_storage(
        Path(args.backup),
        storage_config(Path(getattr(args, "config", DEFAULT_RUNTIME_CONFIG_PATH))),
    )
...
from alpha_operator_framework.cli.maintenance import (
    command_clean_db,
    command_init_db,
    command_storage_backup,
    command_storage_restore,
)
