...
class BacktestDatasetRecord:
    ...
    strategy 粒度 = 生成策略类型 (stage): first_order / semantic_pair / antonym_pair /
    paired_base / survey (模板类库分支)。
    唯一键 (region, universe, delay, dataset_id, strategy): 该组合已回测 → 后续跳过整个组合,
    不再生成+回测, 避免重复消耗额度。
    """
    ...
    universe: str = ""
    delay: int = 1
    dataset_id: str = ""             # 数据集 id; '' = 全字段
    strategy: str = ""               # 生成策略类型 (stage)
    expression_count: int = 0        # 该组合本次生成的表达式数
    backtest_count: int = 0          # 该组合本次实际回测数
    created_at: str = ""
    updated_at: str = ""
...
class DataField:
    ...
    coverage: float = 0.0
    date_coverage: float = 0.0             # 平台 dateCoverage (历史日期覆盖)
    user_count: int = 0
    alpha_count: int = 0
...
class Template:
    ...
    # 占位符→固定 衍生关系: parent_template_id 指向母版模板主键 id; None=原创母版
    parent_template_id: Optional[int] = None
    # 该固定模板有信号的约束记录 (供下一轮优先复用); 列表元素为 dict:
    #   {"field": <数据字段>, "group": <分组维度>, "passed": bool,
    #    "sharpe": float, "fitness": float, "region": str, "universe": str, "note": str}
    signal_constraints: List[dict] = field(default_factory=list)
    active: int = 1
    created_at: str = ""
    updated_at: str = ""
