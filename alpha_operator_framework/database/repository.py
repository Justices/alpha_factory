...
# 聚合算子池: 归一化为统一指纹, 识别"结构相同仅算子不同"的近亲表达式
...
def _isomorphic_fingerprint(expr: str) -> str:
    ...
    用途: 采样层近亲去重 —— 结构相同、仅聚合算子不同的表达式
    (如 vec_stddev vs vec_min) 会得到同一指纹, 避免同组被重复抽出。
    其余部分(ts_算子/窗口/winsorize参数/字段)原样保留, 不参与归一化。
    """
...
class AlphaDatabase:
    """Alpha数据库管理器 — 基于 SQLite WAL 模式与线程安全连接池."""

    # 默认数据库路径: 项目根目录/data/alpha_research.db
    DEFAULT_DB_PATH = Path(__file__).resolve().parents[2] / "data" / "alpha_research.db"

    def __init__(self, db_path: Optional[str] = None, timeout: float = 30.0, wal_mode: bool = True):
        """初始化数据库.

        Args:
            db_path: 数据库文件路径，默认使用 data/alpha_research.db
            timeout: 忙锁等待超时秒数 (默认 30.0)
            wal_mode: 是否开启 WAL 并发写入模式 (默认 True)
        """
    ...
    def _init_database(self):
        ...
        """)
        # 旧库补列 (无 date_coverage 列时追加)
        cols = {r[1] for r in cursor.execute("PRAGMA table_info(datafields)").fetchall()}
        if "date_coverage" not in cols:
        ...
        # 算子级信号统计表 (研究闭环 — 算子信号沉淀, 第6→2 回流; 证据驱动算子挑选)
        cursor.execute("""
        ...
        # 回测数据集记录: 「数据集 + 策略」已回测的历史, 供后续同组合幂等过滤
        cursor.execute("""
        ...
        # template_library: 占位符→固定 衍生关系 (parent_template_id 指向母版主键) + 有信号约束
        ...
        if "parent_template_id" not in template_lib_columns:
            ...
            # 回填: 旧 parent_template(name) → parent_template_id(id)
    ...
    def insert_expression(self, expression: str, settings: Dict, *, expression_origin: str = "",
        ...
        try:
            ...
            # 已存在: 回填 origin(空才填) + batch_id(取最新) + fields(合并) + status(completed优先) + first_operator(空才填)
            cursor.execute("""
    ...
    def catalog_expression(
        ...
        *,
        ...
        Args:
            ...
            backtest_settings: 回测设置 (region/universe/delay/decay/neutralization/truncation),
                合并进 settings JSON 的 "backtest" 键, 供表达式追溯回测参数。
                注意: 表达式按 expression_sha 去重, 首次写入的 settings (含 backtest) 保留,
                后续重复 catalog 或回测链路 (batch_id/status 更新) 均不回写 settings,
                故 backtest 键 = 首次登记时的回测设置; 跨 region 复用追溯走 alpha_details。
        """
        settings = {
            "stage": stage,
            "family": family,
    ...
    def catalog_tasks(
        ...
        Args:
            backtest_settings: 回测设置 (region/universe/delay/decay/neutralization/truncation),
                随每个表达式写入 settings JSON 的 "backtest" 键。
        """
        if not tasks:
            return 0
        conn = self._get_connection()
    ...
    def upsert_backtest_record(self, *, region: str, universe: str, delay: int,
        ...
        """记录「数据集 + 策略」已生成/回测, 幂等 (按唯一键 upsert, 计数累加)."""
        now = self._timestamp()
        conn = self._get_connection()
        conn.execute("""
    ...
    def list_backtest_record_strategies(self, *, region: str, universe: str, delay: int,
        ...
        """返回已回测过的策略(stage)集合, 供 survey 生成前过滤「已回测组合」."""
        rows = self._get_connection().execute(
    ...
    def sample_catalog_expressions(
        ...
        近亲去重 (dedup_isomorphic):
          字段组内随机选取前, 先按同构指纹(聚合算子归一化为 vec_AGG)折叠,
          同一指纹只保留一个代表, 避免 "结构相同仅聚合算子不同" 的表达式
          (如 vec_stddev vs vec_min) 被同组重复抽出。
          关闭: dedup_isomorphic=False (如需要保留全部算子变体)。
        ...
        # 第二层: 批次内按字段组合分组
        out: List[str] = []
        # 全局已选指纹集合: 跨 batch/字段组 跳过已覆盖的同构近亲 (dedup_isomorphic=True 时)
        ...
        # 未填满时从剩余表达式补齐 (注意不超过 batch_cap; 同构指纹已覆盖的跳过)
        if len(out) < limit:
    ...
    def _pick_with_isomorphic_dedup(
        ...
        """从池中选取 limit 个, 默认先做同构折叠(聚合算子归一化, 同指纹只留一个代表).

        同指纹只保留一个, 宁缺毋滥 —— 不足 limit 不在此补足,
        由上层"全局补齐"阶段按同构指纹跳过已覆盖的近亲。
        """
        if limit <= 0:
            return []
        if not dedup:
    ...
    def upsert_operator_signal_stats(self, rows: List[Dict[str, Any]], *,
                                     accumulate: bool = True) -> int:
        """批量 upsert 算子级信号统计行 (研究闭环沉淀, 第6步回流第2步算子选择).

        Args:
            rows: 每行含 operator/region/universe/delay/round 及
                  trials/signal_count/hit_rate/avg_sharpe/max_sharpe/min_sharpe/avg_fitness
            accumulate: True 时累加计数并重算 hit_rate (多轮累积), 语义同
                  upsert_field_signal_stats
    ...
    def get_operator_signal_stats(self, *, region: Optional[str] = None,
                                  ...
                                  min_trials: int = 1,
                                  limit: int = 200) -> List[Dict[str, Any]]:
        """查询算子级信号统计, 按 hit_rate 降序 (证据驱动算子挑选的数据源)."""
    ...
    def deactivate_templates(self, *, template_ids: Optional[Sequence[int]] = None,
        ...
        """标记模板 inactive (active=0), 让 list_templates(active_only=True) 不再返回.

        模板蒸馏淘汰的写入口: 低质量模板 (如嵌套 ts_delta 的差分层叠/增长率二阶) 经回测
        验证零信号后, 标记 inactive 供下一轮 survey 自然跳过 —— 用软删除而非硬编码删除,
        保留可追溯、可恢复 (置回 active=1 即可复活)。
        """
    ...
    def upsert_prune_rule(self, pattern: str, *, pattern_type: str = "prefix",
        ...
        """写入一条淘汰规则 (幂等, 按 pattern+pattern_type 去重)."""
        now = datetime.now().isoformat()
        conn = self._get_connection()
        conn.execute("""
