...
def prepare_super_candidates(
    database: Any, settings: dict[str, Any], *, max_candidates: int = 6
) -> list[dict[str, Any]]:
    candidates = build_super_candidates(
        database.get_candidates_for_super_alpha(),
        SuperAlphaConfig(max_candidates=max_candidates),
        settings,
    )
...
async def simulate_super(
    ...
    for candidate in candidates:
        ...
        try:
            task = {
                **candidate,
                "expression": candidate["candidate_sha"],
                "decay": args.decay,
            }

            def submit(tasks: list[dict[str, Any]]) -> str:
                payload = super_simulation_payload(
                    tasks[0],
                    {
            ...
            batch_id = SimulationTracker(
                database, submit=submit, fetch=lambda _: ({}, 0), detail=lambda _: {}
...
def command_prepare_super(args: argparse.Namespace) -> None:
    ...
    try:
        candidates = prepare_super_candidates(
            database, settings, max_candidates=args.max_candidates
        )
    finally:
        database.close()
...
def command_simulate_super(args: argparse.Namespace) -> None:
    ...
    candidates = (
        payload.get("candidates", payload) if isinstance(payload, dict) else payload
    )
