...
def normalize_platform_url(base_url: str, location: str) -> str:
    if location.startswith(("http://", "https://")):
        return location
    return f"{base_url}{location}" if location.startswith("/simulations/") else f"{base_url}/simulations/{location}"
...
def _platform_tracker(database: Any, *, submit: Any) -> SimulationTracker:
    ...
    def fetch(location: str) -> tuple[dict[str, Any], float]:
        response = brain_client.session.get(normalize_platform_url(brain_client.base_url, location)); response.raise_for_status()
        return response.json() if response.text else {}, float(response.headers.get("Retry-After", 0))
    def detail(alpha_id: str) -> dict[str, Any]:
        response = brain_client.session.get(f"{brain_client.base_url}/alphas/{alpha_id}"); response.raise_for_status(); return response.json()
...
async def simulate(tasks: Sequence[dict[str, Any]], args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for decay, items in by_decay.items():
        for start in range(0, len(items), args.batch_size):
            ...
            if len(batch) < 2:
                results.extend({**task, "status": "PENDING_NEEDS_PAIR"} for task in batch); continue
            settings = {"region": args.region, "universe": args.universe, "delay": args.delay, "decay": decay, "neutralization": args.neutralization, "truncation": args.truncation, "nan_handling": args.nan_handling, "test_period": args.test_period}
            database = open_alpha_database(Path(getattr(args, "config", DEFAULT_CONFIG_PATH)))
            try:
                def submit(batch_tasks: list[dict[str, Any]]) -> str:
                    payload = [{"type": "REGULAR", "settings": {"instrumentType": "EQUITY", "region": args.region, "universe": args.universe, "delay": args.delay, "decay": decay, "neutralization": args.neutralization, "truncation": args.truncation, "pasteurization": "ON", "unitHandling": "VERIFY", "nanHandling": args.nan_handling, "language": "FASTEXPR", "visualization": False, "testPeriod": args.test_period, "maxTrade": "OFF"}, "regular": task["expression"]} for task in batch_tasks]
                    ...
                    return location
                batch_id = _platform_tracker(database, submit=submit).submit(batch, settings)
                results.append({"simulation_batch_id": batch_id, "status": "submitted", "requested_count": len(batch)})
            finally:
                database.close()
    return results
...
def command_simulate(args: argparse.Namespace) -> None:
    ...
    payload = json.loads(Path(args.tasks).read_text(encoding="utf-8")); tasks = payload.get("tasks", payload) if isinstance(payload, dict) else payload
    results = asyncio.run(simulate(tasks, args)); _write_json(Path(args.output), {"settings": vars(args), "results": results})
    print(f"simulation batches={len(results)} output={args.output}")


def command_poll_simulation(args: argparse.Namespace) -> None:
    payload = asyncio.run(poll_simulation_batch(args.batch_id, Path(getattr(args, "config", DEFAULT_CONFIG_PATH))))
    _write_json(Path(args.output), payload); batch = payload["batch"]
