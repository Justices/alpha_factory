...
# ---------------------------------------------------------------------------
# 5. AST 规范化与等价去重剪枝 (回测前)
# ---------------------------------------------------------------------------

def ast_canonical_prune(tasks: Sequence[Any]) -> Tuple[List[Any], List[Any]]:
    """基于 Alpha AST 规范化化简并做等价去重.

    作用:
      1. 自动消除冗余算子 (如 rank(rank(x)) -> rank(x), reverse(reverse(x)) -> x)
      2. 可交换算子重排 (如 a + b 与 b + a 识别为同构)
      3. 剔除语义完全相同的重复公式，避免重复消耗回测配额

    Args:
        tasks: Task 实例列表 (或含 expression 属性/键的对象)

    Returns:
        (kept_tasks, pruned_tasks)
    """
    ...
    for task in tasks:
        raw_expr = task.expression if hasattr(task, "expression") else (task.get("expression") if isinstance(task, dict) else str(task))
        try:
            can_str = to_canonical_string(raw_expr)
            can_sha = get_canonical_sha(can_str)
        except Exception:
            # 解析异常兜底保留原样
            can_str = raw_expr
            can_sha = raw_expr

        if can_sha in seen_shas:
            if isinstance(task, dict):
                pruned.append({**task, "prune_reason": "ast_duplicate", "canonical_expression": can_str})
            else:
                pruned.append(task)
            continue

        seen_shas.add(can_sha)

        # 若化简后表达式有改变，且 task 为 dataclass 则更新 expression
        if can_str != raw_expr:
            if is_dataclass(task):
                updated_task = replace(task, expression=can_str)
                kept.append(updated_task)
            elif isinstance(task, dict):
                kept.append({**task, "expression": can_str, "raw_expression": raw_expr})
            else:
                kept.append(task)
        else:
            kept.append(task)
...
def sandbox_prefilter(
    ...
    """在发起云端仿真前，使用本地向量化沙盒快速预筛淘汰死因子.

    Args:
        tasks: Task 列表
        market_data: 可选的 MarketDataCrossSection
        min_coverage: 最小有效覆盖率 (默认 10%)
        min_abs_ic: 最小绝对 Rank IC 门槛 (默认 0.005)
        min_sharpe: 最小本地 Sharpe 门槛 (默认 0.1)

    Returns:
        (passed_tasks, rejected_tasks)
    """
    ...
    for task in tasks:
        expr = task.expression if hasattr(task, "expression") else (task.get("expression") if isinstance(task, dict) else str(task))
        metrics = engine.evaluate_metrics(expr)

        if not metrics.is_valid or metrics.coverage < min_coverage:
            if isinstance(task, dict):
                rejected.append({**task, "sandbox_reason": f"invalid_or_low_coverage: {metrics.error_message}", "metrics": metrics.to_dict()})
            else:
                rejected.append(task)
            continue

        # 检查是否有微弱预测信号 (IC 或 Sharpe 达标)
        if abs(metrics.rank_ic) >= min_abs_ic or abs(metrics.sharpe) >= min_sharpe:
            if isinstance(task, dict):
                passed.append({**task, "sandbox_metrics": metrics.to_dict()})
            else:
                passed.append(task)
        else:
            if isinstance(task, dict):
                rejected.append({**task, "sandbox_reason": "zero_signal", "metrics": metrics.to_dict()})
            else:
                rejected.append(task)
