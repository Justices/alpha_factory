...
class BrainSessionManager:
    ...
    def persist(self, session: requests.Session) -> None:
        # Session 持久化是 best-effort: 清理临时文件时可能被沙箱安全删除钩子拦截
        # (SAFE_DELETE_FAIL_CLOSED) 或 Windows 文件锁抛 PermissionError, 任何失败
        # 都不能中断已成功的平台请求 —— 内存 cookie 本进程仍有效, 其他进程各自持有副本。
        try:
            self._persist_locked(session)
        except Exception:
            pass

    def _persist_locked(self, session: requests.Session) -> None:
