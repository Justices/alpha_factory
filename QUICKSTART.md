...
### 1. 一键分层地毯式 Alpha 挖掘与自优化 (`mine`)
```bash
# 覆盖指定另类数据集，自动生成 5000+ 表达式，分层抽样，分批回测并流式落库，自动剪枝与正向优化
python alpha_machine.py mine \
  ...
  --execute --output data/gbr_carpet_mining_report.md
```

### 2. 一键前沿学术研报转化与在线终审 (`research`)
```bash
# 直接解析 PDF 论文提取假说，动态映射真实字段，生成 AST 表达式并在线终审评级落库
python alpha_machine.py research \
  --paper papers/2605.09712v1.pdf \
  --region GBR --datasets model30,risk71 \
  --execute --output data/gbr_paper_report.md
```

## 传统三段式工作流 (Survey → Deepen → Submit)
