...
| **10. DDD 投研生命周期与三大核心聚合** | 落地 `ResearchRound`、`ExperimentBatch`、`KnowledgeBase` 领域聚合，4 大纯抽样算法与 2D 跨字段共识后剪枝。 | **✅ P0 (阶段四已落地)** |
| **11. 生产高可用与快照可回放持久化** | 引入 `research_round_snapshots` / `experiment_batch_snapshots`，单查询可答性与重启安全恢复。 | **✅ P0 (阶段四已落地)** |
...
gantt
    ...
    section 阶段四：DDD 重构与生产闭环 (领域聚合与高可用)
    三大领域聚合 (Round / Batch / Knowledge)   :done, p4_ddd, 2026-08-23, 2026-08-23
    4 大纯抽样算法与 2D 跨字段共识后剪枝        :done, p4_sel, 2026-08-23, 2026-08-23
    快照隔离持久化与单查询可答性审计             :done, p4_snp, 2026-08-23, 2026-08-23
```
