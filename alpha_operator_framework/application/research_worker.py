...
class ResearchBatchWorker:
    ...
    def _persist_primary_result(self, result: Any, task: Any) -> None:
        ...
        if result.error or not result.platform_alpha_id:
            self.alpha_database.set_expression_status(
                task.expression, "pending", dict(task.settings)
            )
            return
        ...
        self.alpha_database.save_result_with_checks(
            result.platform_alpha_id, payload, dict(task.settings)
        )
        self.alpha_database.set_expression_status(
            task.expression, "completed", dict(task.settings)
        )
    ...
    def _persist_simulation_result(self, batch: Any, result: Any) -> None:
        ...
        self.alpha_database.record_simulation_result(
            batch.storage_batch_id,
            sequence_no,
            status="failed" if failed else "completed",
            alpha_id=result.platform_alpha_id or "",
            result=dict(result.raw_details or {}),
            error_message=result.error
            or ("platform returned no alpha ID" if failed else ""),
        )

    def _event(
        self, event_type: EventType, round_id: str, payload: dict[str, Any]
    ) -> int:
        return self.event_store.append(
            Event.create(event_type, round_id, payload, actor="worker:research-batch")
        )
    ...
    def _policy(self, round_id: str) -> ResearchPolicy:
        for event in self.event_store.read_stream(round_id):
            if event.event_type is EventType.POLICY_CREATED:
                ...
                payload["prohibited_patterns"] = tuple(
                    payload.get("prohibited_patterns", ())
                )
    ...
    def _transition(self, batch: Any, target: BatchState) -> None:
        ...
        if not record.accepted:
            raise ValueError(
                f"invalid batch transition {record.from_state} -> {target}"
            )
    ...
    def _summary(
        self, batch: Any, status: str, knowledge_version: int = 0, templates: int = 0
    ...
    def _record_retry_fact(
        self, batch: Any, round_id: str, task_ids: list[str], state: BatchState
    ) -> None:
        """Persist the retry projection facts required to rebuild a batch."""
        self._event(
            EventType.MONITORING_OBSERVED,
            round_id,
            {
                "retry": {
                    "state": state.value,
    ...
    def process_round(self, round_id: str) -> ResearchCycleSummary:
        ...
        missing = [
            task for task in batch.tasks.values() if task.task_id not in batch.results
        ]
        due = [
            task
            for task in missing
            if task.next_retry_at is None
            or datetime.fromisoformat(task.next_retry_at) <= now
        ]

        logger.info(
            "回合 %s: 共有 %d 个任务配置，其中已完成=%d, 未完成=%d, 满足到期执行条件=%d",
            round_id,
            len(batch.tasks),
            len(batch.results),
            len(missing),
        ...
        try:
            ...
            for offset in range(0, len(due), BACKTEST_BATCH_SIZE):
                shard_results = list(
                    self.backtest_gateway.run_backtests(
                        due[offset : offset + BACKTEST_BATCH_SIZE]
                    )
                )
                ...
                for result, task in completed_events:
                    self._event(
                        ...
                        round_id,
                        {
                            ...
                            "alpha_id": result.platform_alpha_id,
                            "sharpe": result.sharpe,
                            "fitness": result.fitness,
                            "turnover": result.turnover,
                            "margin": result.margin,
            ...
            delay_seconds = self._retry_after_seconds(
                error
            ) or self._policy_backoff_seconds(policy, attempts)
            retry_at = now + timedelta(seconds=delay_seconds)
            task_ids = [task.task_id for task in retry_due]
            batch.record_retry(
                task_ids, next_retry_at=retry_at.isoformat(), error=str(error)
            )
            self._record_retry_fact(
                batch, round_id, task_ids, BatchState.PARTIAL_FAILED
            )
        ...
        if failures:
            logger.warning(
                "本回测周期发现有 %d 个子因子任务在平台端执行失败", len(failures)
            )
            ...
            for result in retryable_failures:
                ...
                batch.record_retry(
                    ...
                    error=result.error or "platform execution failed",
                )
                self._event(
                    EventType.MONITORING_OBSERVED,
                    round_id,
                    {"task_id": result.task_id, "platform_failure": result.error},
                )
            for result in terminal_failures:
                task = batch.tasks[result.task_id]
                if self.alpha_database is not None:
                    self.alpha_database.set_expression_status(
                        task.expression, "failed", dict(task.settings)
                    )
                self._event(
                    EventType.MONITORING_OBSERVED,
                    round_id,
        ...
        for result in results:
            task = batch.tasks[result.task_id]
            logger.info(
                "因子 %s 回测正常完成。Sharpe=%.2f, Alpha ID=%s",
                task.expression,
                result.sharpe,
                result.platform_alpha_id,
            )
        ...
        if len(batch.results) != len(batch.tasks):
            ...
            self._record_retry_fact(
                batch, round_id, outstanding, BatchState.PARTIAL_FAILED
            )
            self._transition(batch, BatchState.PARTIAL_FAILED)
        ...
        from alpha_operator_framework.research.template_correlation import (
        ...
        for task_id, result in batch.results.items():
            correlation = platform_correlation(
                result.self_correlation, result.production_correlation
            )
            passed = correlation <= policy.template_platform_max_correlation
            self._event(
                EventType.CORRELATION_CHECKED,
                round_id,
                {
                    "task_id": task_id,
                    "alpha_id": result.platform_alpha_id,
                    "self_correlation": result.self_correlation,
                    "production_correlation": result.production_correlation,
                    "max_abs_correlation": correlation,
                    "threshold": policy.template_platform_max_correlation,
                    "passed": passed,
                },
            )
            if not passed:
                ...
                self._event(
                    EventType.CANDIDATE_RETIRED,
                    round_id,
                    {
                        "task_id": task_id,
                        "reason": "platform_correlation",
                        "max_abs_correlation": correlation,
                        "threshold": policy.template_platform_max_correlation,
                    },
                )
                logger.warning(
                    "因子 %s 因相关度过高被剔除过滤 (相关度=%.4f > 限额=%.4f)",
                    result.platform_alpha_id,
                    correlation,
                    policy.template_platform_max_correlation,
                )
        ...
        for evaluation in evaluate_batch(batch, policy):
            ...
            self._event(
                EventType.VALIDATION_COMPUTED,
                round_id,
                {
                    "task_id": evaluation.task_id,
                    "verdict": evaluation.verdict,
                    "pareto_rank": evaluation.pareto_rank,
            ...
            if evaluation.verdict == "READY" and not evaluation.pruned:
                ...
                logger.info(
                    "发现符合优质提交条件的因子 candidate=%s (alpha_id=%s)，开启 6 维证据链路自动审批裁决...",
                    evaluation.task_id,
                    result.platform_alpha_id,
                )
                if self.evidence_gateway is not None:
                    from alpha_operator_framework.knowledge.submission import (
                        SubmissionCase,
                    )

                    case = SubmissionCase.from_result(
                        result, self.evidence_gateway.evidence_for(result)
                    )
                    approval = case.approve()
                    self._event(
                        EventType.DECISION_APPROVED
                        if approval.is_approved
                        else EventType.DECISION_REJECTED,
                        round_id,
                        {
                            "task_id": evaluation.task_id,
                            "alpha_id": result.platform_alpha_id,
        ...
        templates_by_candidate = {
            candidate.candidate_id: candidate.template_id
        ...
        distilled = distill_templates(
            batch,
            min_support=policy.template_min_support,
            min_sharpe=policy.template_min_sharpe,
            min_fitness=policy.template_min_fitness,
        )
        promoted_templates = []
        if self.alpha_database is not None:
